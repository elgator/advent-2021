...
# ╔═╡ c29b0e89-67a9-49a1-8030-8a86d9f0bb6e
function part2(boards, ball_seq)
	@show "-----------------------------"
	wins = false
	board_idxs = collect(1:length(boards))
	winning_board = nothing
	winning_ball = nothing
	for ball in ball_seq
		@show ball
		to_delete = []
		for (i, idx) in enumerate(board_idxs)
			wins = new_ball!(boards[idx], ball)
			if wins
				winning_board = idx
				winning_ball = ball
				push!(to_delete, i)
		...
		deleteat!(board_idxs, to_delete)
		length(board_idxs)==0 && break
	end
	return  sum_of_unmarked(boards[winning_board]) * winning_ball
end
...
# ╔═╡ 0a0dd22d-71f2-428b-8b6b-d62bac676f26
part2(boards_test, bingo_seq_test)

# ╔═╡ 352d9a39-bdaa-44f6-a1ab-305d43b3358d
ans2 = part2(boards, bingo_seq)
